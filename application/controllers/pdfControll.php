...
class pdfControll extends CI_Controller {
    ...
    public function get_surat($kodemk){
        $data=$this->Menu_model->getUnduh($kodemk);

        if($data['nilai'] > 50){
            $this->SuratKelulusan($kodemk);
        }else{
            $this->SuratTidaklulus($kodemk);
        }
    }

    public function SuratTidaklulus($kodemk){
        ...
        // Memberikan space
        $pdf->SetFont('Times','B',14);
        $pdf->Cell(169,8,'TIDAK LULUS',0,4,'C');
        // Memberikan space
        $pdf->SetFont('Times','',12);
        $isi=$this->Menu_model->getMatkul($kodemk);
        $pdf->Cell(55,7,'Dalam mengikuti Tugas Besar,',0,0,'L');$pdf->Cell(60,7,$isi['namamk'],0,1);
        ...
        $pdf->Cell(169,7,'',0,1);
        $pdf->SetFont('Times','',12);
        $pdf->Cell(169,7,'Lampung Selatan,',0,0);$pdf->Cell(60,7,date('d-m-Y'),0,1);
    ...
    public function SuratKelulusan($kodemk){	
		$pdf = new FPDF('P','mm','A4');	//L = lanscape P= potrait		
		$pdf->SetLeftMargin(20);
        $pdf->SetTopMargin(0);
        // membuat halaman baru
        ...
        $pdf->Cell(169,7,'',0,1);
        $pdf->SetFont('Times','',12);
        $pdf->Cell(169,7,'Lampung Selatan,',0,0); $pdf->Cell(169,7,date('d-m-Y'),0,1);
        $pdf->Cell(169,7,'Ketua Program Studi/Yang Mewakili',0,1,'R');
        $pdf->Cell(169,20,'',0,3);
        $pdf->Cell(104,4,'',0,0);$pdf->Cell(50,5,'Ir.Titi Liliani Soedirjo, M.Sc','B',0,'L');
        $pdf->Cell(60,5,'',0,1);
